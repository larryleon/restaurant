...
public class Database extends Object {
	...
	public static Vector getSaleRecords(String resid, String cusid,
			java.sql.Date begin) {
		/**
		 * ss[0]=resid; ss[1]=customerID.getText().trim();
		 * ss[2]=firstName.getText().trim(); ss[3]=lastName.getText().trim();
		 * ss[4]=un1.getText().trim();//street
		 * ss[5]=un2.getText().trim();//aparty ss[6]=city.getText().trim();
		 * ss[7]=state.getSelectedItem().toString().trim();
		 * ss[8]=un4.getText().trim();//zip ss[9]=phone_Res.getText().trim(); //
		 */
		Vector data = new Vector();
		double to = 0;
		// int ti=0;
